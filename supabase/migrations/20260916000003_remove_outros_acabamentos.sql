-- ----------------------------------------------------------------------------
-- Remove outros_acabamentos do cálculo de margem — a coluna some da tela
-- ("Outros"), substituída pelo detalhamento em fotolito/gravacao_tela/corte.
-- Continua existindo em product_costs (estava zerada pra todas as peças),
-- só não entra mais na conta de direct_cost.
-- ----------------------------------------------------------------------------
...
components as (
  select
    ...
    (
      ...
      + coalesce(pc.fotolito, 0::numeric) + coalesce(pc.gravacao_tela, 0::numeric) + coalesce(pc.corte, 0::numeric)
